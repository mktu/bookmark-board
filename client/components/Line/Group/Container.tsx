...
const Container : React.VFC = ()=> {
    const { groups, defaultGroup, error, setDefaultGroup, fetching, posting, updateDefaultGroup, onClose, idToken } = useGroups()
    ...
    const cancel = (
        <OutlinedButton onClick={onClose} disabled={posting} className='w-full h-16 bg-white'>{idToken || 'NoId'}</OutlinedButton>
    )
    return (
