...
export const renderArrowNext = (clickHandler: () => void, hasNext: boolean) => (
    <SvgIconButton aria-label='next' disabled={!hasNext} className='absolute top-1/2 right-0 z-50 h-8 w-8 stroke-2' onClick={clickHandler}>
...
export const renderArrowPrev = (clickHandler: () => void, hasPrev: boolean) => (
    <SvgIconButton aria-label='prev' disabled={!hasPrev} className='absolute top-1/2 left-0 z-50 h-8 w-8 stroke-2' onClick={clickHandler}>
